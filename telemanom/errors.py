...
from telemanom.ts_metric import TSMetric
    ...
    if len(E_seq) > 0:
        ...
        for e_seq in E_seq:
            ...
            for i, a in enumerate(E_seq_test):
                ...
                if (a[0] <= e_seq[0] <= a[1]) or (a[0] <= e_seq[1] <= a[1]) or \
...
def evaluate_sequences_new_metric(E_seq, anom, logger):
    metric = TSMetric(
    ...
    length = int(anom['num_values'])
    values = ast.literal_eval(anom['anomaly_sequences'])
    values_real = np.zeros((length), dtype=np.int)
    for idx, seg in enumerate(values):
        values_real[seg[0]:seg[1]] = 1
    values_predicted = np.zeros((length), dtype=np.int)
    for idx, seg in enumerate(E_seq):
